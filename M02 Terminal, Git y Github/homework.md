![AvivateLogo](../Assets//logoBannerAvivate.png)
...
El objetivo de este ejercicio es que crees un nuevo repositorio llamado "_**RepositorioAvivate**_".
...
En este ejercicio debes ingresar dentro del repositorio y crear una nueva carpeta con el nombre "_CarpetaAvivate_". Utiliza los comandos de más arriba.
...
Si llegas a recibir un mensaje por consola similar a este:
```bash
...
Deberás primero ejecutar los siguientes comandos:
```bash
git config --global user.email "you@example.com"
git config --global user.name "Your Name"
```
reemplazando _you@example.com_ por tu email registrado en *Github* y _Your name_ por el nombre de usuario con el que figuras en *GitHub*

*Y LUEGO:* vovle a ejecutar los comandos de add, commit y push.

Recuerda que todos estos comandos son necesarios. Si tienes alguna duda, no dudes en avisarnos!
